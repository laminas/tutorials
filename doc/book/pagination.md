...
we will be using the [DbSelect paginator adapter](https://zendframework.github.io/zend-paginator/usage/#the-dbselect-adapter)
...
Next, we need to tell the album controller to return a `Pagination` object
...
`page` parameter. We are also telling the paginator we want to display 10
objects per page.

## Updating the View Script

Now, let's just tell the view script to iterate over the `pagination` view
variable, rather than the `albums` variable:
